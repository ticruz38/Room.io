...
export default class Timeline extends React.Component<any, any> {
  ...
  onClick = (event) => {
    ...
    state.current = state.today + (x * 10);
    this.state.x = x;
  };
  ...
  render() {
    ...
    return (
      <div className='timeline' ref = {(item) => { this.width = item ? item.clientWidth : time}}> 
        <svg id='timeline' className='command' viewBox="0 0 8640 800" onWheel={ (e: any) => state.onWheel(e) } onClick={this.onClick}>
          ...
          <text 
            ...
            fontSize='200'
          >
            { moment.unix(state.current).format('L') }
          </text>
          <text 
            textAnchor='middle' 
            x='4320' 
            ...
            fontSize='200'
          >
            { moment.unix(state.current).format('LT') }
          </text>
        </svg>
